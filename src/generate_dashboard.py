          ...
          html += `
            <div style="margin-bottom:8px">
              <img src="${site.sentinel.image_miniature}"
                   alt="Image Sentinel-2 fausse couleur"
                   title="Fausse couleur NIR — les algues apparaissent en rouge"
              ...
              <div style="font-size:11px;color:var(--texte-faible);margin-top:4px">
                Fausse couleur NIR · ${site.sentinel.image_la_plus_recente || ""}
              </div>
            </div>`;
      ...
      if (site.airbreizh) {
        ...
        if (ab.mesure && ab.mesure.derniere_mesure_ug_m3 !== null) {
          ...
          html += `
            ...
            <table class="facteurs-table">
              <tr><th>Station</th><td>${escapeHtml(ab.station ? ab.station.nom : "—")}</td></tr>
              <tr><th>Dernière mesure</th><td>${escapeHtml(m.horodate_derniere_mesure || "—")}</td></tr>
